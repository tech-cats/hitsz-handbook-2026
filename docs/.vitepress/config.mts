...
const HIDDEN_SECTION_DIRS = new Set<string>()
const HIDDEN_SECTION_FILES = new Set([
  'guide/fireside/current-affairs-preface.md',
  'guide/fireside/post-pandemic-era.md',
  'guide/fireside/knowledge-ai.md',
  'guide/fireside/self-organization.md',
  'guide/fireside/rationality.md',
])
const HIDDEN_SECTION_GLOBS = [
  ...[...HIDDEN_SECTION_DIRS].map((dir) => `${dir}/**`),
  ...HIDDEN_SECTION_FILES,
]
...
function collectSiteStats() {
  ...
  const walk = (dir: string, relativeDir = '') => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      } else if (HIDDEN_SECTION_FILES.has(relativePath)) {
        continue
      } else if (
      ...
      '/guide/': [
        { text: '前言', link: '/guide/' },
        {
          ...
          items: [
            ...
            { text: '综测与保研', link: '/guide/practical/graduate-recommendation' },
          ],
        },
        {
          text: '行事',
          items: [
            ...
            { text: '如何学习计算机技术', link: '/guide/relationships/learning-computer-technology' },
          ],
        },
        {
          text: '围炉夜话',
          items: [
            {
              text: '路径之问',
              items: [
                { text: '访谈：理与工', link: '/guide/fireside/interview-science-engineering' },
              ],
            },
          ],
        },
